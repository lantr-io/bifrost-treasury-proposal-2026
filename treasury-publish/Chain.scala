...
object Chain:
    ...
    /** Read a raw-hex ed25519 keypair; derive the pubkey from the private key,
      * assert it matches the .vkey file. */
    ...
    def loadBlockfrostKey(net: Net): String =
        val key = sys.env.get("BLOCKFROST_PROJECT_ID").filter(_.nonEmpty).getOrElse {
            Seq(Path.of(".env"), Path.of("../.env")).collectFirst {
    ...
    /** Conway: a stake account is "registered" once and persists across proposal
      * cycles; a fresh deployment must not re-register it. 404 => never seen. */
    ...
    /** Blockfrost serves script CBOR single-wrapped (`bytes(flat)`), which is
      * exactly what Script.PlutusV3 expects. */
