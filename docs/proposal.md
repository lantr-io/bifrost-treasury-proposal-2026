...
This proposal extends Scalus from a development environment into a full application platform: one coherent stack to build, verify, launch, and scale Cardano applications, with it's own L1 node, native L2 integration, formal verification, and production-grade runtime capabilities.

Lantr Engineering requests ₳8,503,000 (including 10% contingency) from the Cardano Treasury for a 12-month delivery period (June 2026 to May 2027).
...
### Strategic vision: Scalus - Cardano application platform
...
This pattern is not unique to Cardano. Blockchains that reached enterprise environments repeatedly converged on the JVM stack:
...
Scalus is built in Scala. It gives Cardano a native path into the JVM ecosystem: one coherent application platform for Scala, Java, and Kotlin teams, built to fit the environments where finance and fintech already live.

**What about the existing Cardano Java toolset?**

Cardano’s JVM ecosystem is not a blank slate. Projects such as CCL and Yaci (DevKit, Store) already provide important building blocks for JVM developers, and Scalus interoperates with that ecosystem.

What remains missing is an integrated product experience: one stack that brings smart contracts, application runtime, L1 node, and L2 scaling together.
...
Cardano has funded several node implementations: reference node in Haskell, Amaru in Rust, Dingo in Go. They primarily optimise for SPOs and block production. 

Scalus targets a different audience: mission-critical applications and protocols that need their own sovereign chain access and cannot afford to depend on third-party providers for reading chain state or submitting transactions.
...
#### The same node, every phase of the product lifecycle

Building with Scalus means using the same node across the product lifecycle. Switch the configuration, and the it serves the current need:
...
* Launching testnet? Same node connects to Preview or Preprod, bootstrap from Mithril, and launch in minutes.
* Releasing to production? Run same node against mainnet with security, resilience, and observability built in.
* Need scaling? Activate Hydrozoa L2. Plus, the same L1 node as the rest of your application.

Test and production infrastructure are the same code. No parallel infrastructure. No environment drift. One integrated stack.

#### Extending beyond JVM: JavaScript and TypeScript

While JVM is Scalus’s primary target, its architecture extends beyond a single ecosystem. Scalus already exports core node capabilities, including ledger rules, transaction validation, Plutus VM evaluation, and the in-memory node emulator, into JavaScript and TypeScript.

Today, Scalus.js is integrated into MeshJS SDK and Evolution SDK, providing script evaluation, cost calculation, and rapid testing capabilities. This proposal extends that path further with advanced devnet capabilities, and, where practical, data access and more node functionality directly in pure JavaScript and TypeScript.
...
Cardano’s long-term success depends on applications people and businesses actually use. Scalus contributes directly by reducing the complexity of building and operating serious applications on Cardano, helping more teams move from idea to production.
...
This is the kind of work treasury is well placed to fund: shared application infrastructure that benefits the ecosystem beyond a single vendor or product team.

Over time, we expect maintenance to be supported by a broader mix of sources rather than Treasury alone (more details in Future Maintenance Costs section).
...
| KPI | Alignment | Our contribution |
|-----|-----------|---------------------|
| Reliability: Monthly Uptime (6 epochs) | Yes, partially | Application reliability improves when teams can run their own L1 node instead of depending on third-party providers.|
| Operational Resilience: Alternative Full Node Clients | Yes | Scalus contributes an alternative Scala-based node implementation for JVM and native environments, focused on mission-critical applications. A relay-capable node is planned for May 2027, with ongoing collaboration through the Node Diversity initiative. |
...
| **L1 node** | Limited data-node capabilities, including chain following, UTxO indexing, and bootstrapping support | Embedded and standalone L1 node, relay capabilities, broader chain access, and JVM/native deployment |
...
As a result, the 2026 Scalus proposal uses direct on-chain submission, quarterly milestones, more outcome-focused delivery, conservative ADA budgeting, partial hedging into stable assets, refundable contingency, continued third-party assurance, and the same commitment to public reporting.
...
The proposal delivers four milestones over 12 months (June 2026 to May 2027). 
...
#### M1 — Applications with sovereign chain access (June-Aug 2026)

**Objective**: Establish the foundation for self-sovereign applications: embedded L1 node and a reactive runtime.
...
#### M2 — Applications with native L2 scaling (Sep-Nov 2026)
...
#### M3 — Formal verification + advanced devnet (Dec 2026-Feb 2027)
...
* Operate production-grade DeFi protocols with reliable transaction submission (bridge, optimistic roll-up)
* Build MEV-aware systems (market makers, arbitrage bots via mempool watcher)
* Launch standalone JVM/binary relay node

By May 2027, Scalus brings its four pillars together into one integrated platform for the next generation of Cardano applications.

(*) Hydrozoa/Gummiworm L2 and the Blaster formal verification framework are third-party projects. Milestones M2 and M3 depend on those integrations, and Lantr will work closely with  teams to manage that dependency risk.
(**) Milestones M1-M3 progressively deliver key node components required for relay-capable operation. M4 is intended to finalise and harden those capabilities.
...
* At least **2 demonstrated deployment using Scalus-owned chain access** in a production or production-like environment
...
Funding allocations are derived from effort estimates across the core workstreams, valued at a market rate of \$210,000 per FTE-year for senior software engineers and senior product leadership with deep DLT (Distributed Ledger Technology) expertise. 
...
* Cardano L1 node and infrastructure (embedded and standalone deployment, broader chain access, relay capabilities, and JVM/native targets)
...
Developer documentation, learning materials, migration guides, smart contract and dApp blueprints, pilot use-case materials, training programs, and work that improves AI-assisted development readiness around the Scalus platform.
...
| Representing | Member |
|--------------|--------|
| Community    | XXXXX — DRep, SPO |
| Platform     | XXXXX (YYYYYY)  |
| Node         | XXXXX (YYYYYY) | 
...
An external financial auditor will scrutinize Scalus's finances and treasury management, with the goal to publish a resulting report in Q3 2027. It will remunerated with funds explicitly allocated in this proposal for that purpose.
...
|------|-----------------------|
| Disburse| Lantr + any 1 board member |
...
- [x] **a clear withdrawal purpose**: funding the development of Scalus application paltform for the 12 months period;
- [x] **a period for delivery of proposed activities which the withdrawal shall be used for**: June 2026 - May 2027, with a milestone breakdown and detailed scope provided in appendix;
...
- [ ] **circumstances under which the withdrawal might be refunded to the Cardano Treasury**: TBD


#### Article II - Section 7.2: past withdrawal(s) disclosure

> "Treasury Withdrawals" actions shall disclose whether the prospective recipient of the "Treasury Withdrawals" action has received ada from the Cardano Treasury within the last 24 months.
...
> A Net Change Limit must be set. "Treasury Withdrawals" actions must not exceed the Net Change Limit for that period.
...
- [x] periodic independent audits are explicitely detailed in Budget section; 
- [x] quarterly reports submitted as a part of the milestone disbursements.

#### Article II - Section 7.5: administration

> "Treasury Withdrawals" actions shall designate one or more administrators responsible for monitoring how the funds are used, and ensuring the deliverables are achieved.
...
- [x] This proposal withdraw funds in a separate script accounts which each provides a public oversight of all operations, and ensures that funds cannot be delegated to an SPO and can only be delegated to the always-abstain drep.
In addition to the on-chain guarantees provided by the smart contracts, we also commit to an off-chain journaling in the same fashion as done for the first withdrawal.
...
- [ ] This proposal directly supports the Cardano 2030 Strategic Framework, TBD
- [ ] Measurable adoption indicators have been defined to provide visibility into ecosystem-level KPI contributions (TVL, monthly transactions, MAU). TBD
...
---
### Annex 4: Partners and project integrations
